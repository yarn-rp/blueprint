 ...
 */
@injectable()
export class DeleteBlueprintEventUseCase {
  /**
   * Crates a new instance of the DeleteBlueprintUseCase use case.
  ...
  /**
   * Executes the DeleteBlueprintUseCase use case.
  ...
  async execute(event: Event, uid: string): Promise<void> {
    await this.blueprintLocalRepository.deleteEvents([event], uid);
  }
}
